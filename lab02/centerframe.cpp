...
void CenterFrame::createUserCommandArea()
{
    ...
    p.fill(BACKGROUND_COLOR);
    //菱形的四个顶点
    QPointF ptl1(-3+p.size().width(),p.size().height()/2);
    QPointF ptl2(p.size().width()/2,3);
    QPointF ptl3(3,p.size().height()/2);
    QPointF ptl4(p.width()/2,-3+p.size().height());
    QVector<QPointF> ptsl;
    ptsl<<ptl1<<ptl2<<ptl2<<ptl3<<ptl3<<ptl4<<ptl4<<ptl1;
    painter.drawLines(ptsl);
...
void CenterFrame::createUI()
{
    ...
    connect(edtText,&QLineEdit::textEdited,this,&CenterFrame::on_edtTextEdited);   // 连接文本输入时的处理槽函数
    // 右侧垂直布局
    vLayout = new QVBoxLayout();
    vLayout->addWidget(group);
    vLayout->addWidget(edtText);
    vLayout->addStretch();//平均分配
    vLayout->setMargin(5);//控件与窗口的左右边距
    // 创建绘图区
