...
void DrawWidget::paintEvent (QPaintEvent *)
{
    ...
    painter.drawPixmap (QPoint(0, 0), *pix);//添加图片
}

void DrawWidget::resizeEvent (QResizeEvent *event)//处理窗口大小变化时的resize事件
{
...
void DrawWidget::drawShape(const QPointF ptStart,const QPointF ptEnd,const ST::ShapeType drawType)
{
    ...
    case ST::Triangle:{
        ...
        break;
    case ST::Diamond:{
        QPointF pointl1(ptStart.x(),(ptStart.y()+ptEnd.y())/2);
        QPointF pointl2( (ptStart.x()+ptEnd.x())/2,ptStart.y());
        QPointF pointl3( ptEnd.x(),(ptStart.y()+ptEnd.y())/2);
        QPointF pointl4( (ptStart.x()+ptEnd.x())/2,ptEnd.y());
        QVector<QPointF> points;
        points<<pointl1<<pointl2<<pointl3<<pointl4;
        // 画多边形
        painter.drawPolygon(points);
    }
        break;
